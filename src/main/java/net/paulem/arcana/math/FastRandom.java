...
import net.paulem.arcana.utils.ClassUtils;
 ...
 * <p>
 * XORShift implementation from <a href="http://maths.uncommons.org/">...</a>
 * Mallet methods from <a href="http://mallet.cs.umass.edu/">...</a>
 * ... i have no idea what license status that leaves this file
 * <p>
 * See discussions e.g.
 * <a href="http://stackoverflow.com/questions/453479/how-good-is-java-util-random">...</a>
 * <a href="http://www.cs.gmu.edu/~sean/research/mersenne/">...</a>
 * <p>
 ...
 *
 * @author Brendan O'Connor (<a href="http://brenocon.com">..</a>),">Jan 2012, <a href="https://gist.github.com/4561065">...</a>
 */
public class FastRandom implements Serializable {
    public static final FastRandom INSTANCE = of();

    public static FastRandom of() {
        return SystemUtils.isJavaVersionAtLeast(JavaVersion.JAVA_17) ? getJ17FastRandom() : new FastRandom();
    }

    public static FastRandom getJ17FastRandom() {
        try {
            @Nullable
            Class<?> clazz = ClassUtils.getClass(FastRandom.class.getName() + "J17").orElse(null);
            if (clazz != null && FastRandom.class.isAssignableFrom(clazz)) {
                return (FastRandom) clazz.getDeclaredConstructor().newInstance();
            }
        } catch (Throwable ignored) {
            // If we can't instantiate the J17 version, fall back to the regular FastRandom.
        }
    ...
    static private final AtomicInteger uniquifier = new AtomicInteger(0);  // so multiple RNG's created in the same process are forced unique
     ...
     */
    protected FastRandom() {
        this(seedFromTimeHost());
    }


    protected FastRandom(long seed) {
    ...
    static byte[] seedFromTimeHost() {
        ...
        String s = t + " " + uniquifier.incrementAndGet() + " " + ManagementFactory.getRuntimeMXBean().getName();
...
//			System.out.println(y);
    ...
    public double nextGaussian() {
        if (!haveNextGaussian) {
            ...
            double x1, x2;
            double sqrted = Math.sqrt(-2 * Math.log(v1));
            x1 = sqrted * Math.cos(2 * Math.PI * v2);
            x2 = sqrted * Math.sin(2 * Math.PI * v2);
            nextGaussian = x2;
